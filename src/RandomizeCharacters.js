...
const RandomizeCharacters = props => {
  ...
  let randomshit = Math.floor(Math.random() * Math.floor(props.le));

  // generate a random number between 0 and 3
  const randomNumberHouse = Math.floor(Math.random() * 3);
  console.log(randomNumberHouse);
  // use the random number to pick a house from the 4 houses
  const randomHouse = props.gotCharactersArray[randomNumberHouse];
  ...
  const randomCharacter = randomHouse[randomNumberCharacter];
  console.log(randomCharacter);

  return <div>{/* <h2>{randomCharacter.name}</h2> */}</div>;
};
